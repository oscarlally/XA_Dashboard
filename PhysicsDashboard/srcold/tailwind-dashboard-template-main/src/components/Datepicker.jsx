...
function Datepicker({ align, onDateChange }) {
  ...
  const today = moment().toDate(); // Get today's date
  ...
  const options = {
    ...
    onChange: (selectedDates, dateStr, instance) => {
      ...
      // Log the selected dates to check if they are correct
      console.log('Selected Dates in Datepicker:', selectedDates);

      // Call the provided callback with the formatted dates
      if (onDateChange) {
        const formattedDates = selectedDates.map(date => moment(date).format('DD_MM_YYYY'));
